  ...
  useEffect(() => {
    const fetchWeatherForFavorites = async () => {
      ...
      for (const city of favorites) {
        try {
          // Call your weather backend, not favorites endpoint
        ...
        {favorites.length === 0 ? (
          <div className="empty-message">
            ...
            <p className="exploreLinkTxt">
              <Link to="/exploreCities" className="explore-message">
                Explore Cities
              </Link>
            </p>
          </div>
          ...
          favorites.map((city) => {
            ...
            return (
              <div
                key={city}
                className={`favorite-city-card ${getWeatherTheme(weather.desc)}`}
              >
                <Link to={`/weather/${encodeURIComponent(city)}`}>
                  <h4>{city}</h4>
                  <div className="temp">
